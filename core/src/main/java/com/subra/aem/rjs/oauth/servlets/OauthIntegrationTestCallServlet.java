...
@Component(service = Servlet.class, property = {Constants.SERVICE_DESCRIPTION + "= Integration Profile Test Servlet",
        "sling.servlet.methods=" + HttpConstants.METHOD_GET, "sling.servlet.paths=" + "/bin/rjs/oauth/profile"})
public class OauthIntegrationTestCallServlet extends SlingAllMethodsServlet {

    private static final Logger LOGGER = LoggerFactory.getLogger(OauthIntegrationTestCallServlet.class);
    ...
    protected void doGet(SlingHttpServletRequest request, SlingHttpServletResponse response) throws ServletException, IOException {
        ...
        try (CloseableHttpClient client = HttpClientBuilder.create().build()) {
            HttpGet get = new HttpGet("http://localhost:4502/libs/oauth/profile");
            get.addHeader(CONTENT_TYPE, CONTENT_TYPE_URL_ENCODED);
            if (!Boolean.valueOf(request.getParameter("noheader"))) {
                integrationService.updateAuthorizationHeader(get);
            }
            HttpResponse httpResponse = client.execute(get);
            if (httpResponse.getStatusLine().getStatusCode() != 200 && !StringUtils.containsIgnoreCase(httpResponse.getFirstHeader("content-type").getValue(), "json")) {
