...
public class AccessControlUtils {
     ...
     * Please note, that calling {@link Session#save()} is required
     ...
     * a call to {@link Session#save()} is required in order
    ...
    public static boolean addAccessControlEntry(Session session, String absPath,
        ...
        if (acl != null && acl.addEntry(principal, privileges, isAllow)) {
            ...
            return true;
        } //else: not modified OR no acl found.
     ...
     * Please note, that {@link Session#save()} is required in order
     * to persist the changes.
     *
     * @param session The editing session.
     * @param absPath The absolute path of the target node
     ...
     * @param privileges    list of privileges to set by name (see {@link Privilege})
    ...
    public static boolean clear(Session session, String absPath, String principalName) throws RepositoryException {
        ...
        JackrabbitAccessControlList acl = getACLFromExistingPolicies(absPath, acm);
        Principal principal;
        if (acl != null) {
            if (principalName == null) {
                ...
                return (principal = getPrincipal(session, principalName)) != null && removeExistingEntriesForPrincipal(absPath, acm, acl, principal);
            }
        }
        return false;
    }

    private static boolean removeExistingEntriesForPrincipal(String absPath, AccessControlManager acm, JackrabbitAccessControlList acl, Principal principal) throws RepositoryException {
    ...
    private static JackrabbitAccessControlList getACLFromExistingPolicies(String absPath, AccessControlManager acm) throws RepositoryException {
